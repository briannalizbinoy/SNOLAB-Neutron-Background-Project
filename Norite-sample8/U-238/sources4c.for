c***********************************************************************
c                                                                      *
c   SOURCES: Version 4C       (Last Edited: 03/27/02 EFS)              *
c                                                                      *
c***********************************************************************
c                                                                      *
c Written By:                                                          *
...
c  File Structure
c-----------------
      open(unit=1,file='tape1',status='old')
...
c=======================================================================
      ...
      subroutine homo(title,idd,id,erg)
...
c---------
     ...
     1aq(300),el(20), ep(200),f(20,200), eal(30), fal(30), e(1100), x(11
     ...
     5 ,totlev(20),sl(750,20),etopl(20),ebotl(20)
      common /names/ jsm
...
c-------------------
 ...
 1900 format('SOURCES 4C Calculation',/,'<<<<<<<<<<<>>>>>>>>>>>',//)
 ...
 3615 format(' E (MeV)    (a,n)      (sf)       (dn)       Total')
 3620 format(1x,9('-'),2x,9('-'),2x,9('-'),2x,9('-'),2x,9('-'))
 3630 format(1p1e10.3,1p4e11.3)
 3635 format(' Total    ',1p4e11.3)
c***********************************************************************
...
c----------------------------------------------------------------
 ...
 1230 se1=sqrt(en(1))
      ...
      c3=(sen1+sbaso4)/sa
      c4=(se1+sbaso4)/sa
      if (c1.lt.0..and.c2.lt.0.) eft=0.5*(ERF(-c1)-ERF(-c2))
      if (c1.lt.0..and.c2.ge.0.) eft=0.5*(ERF(-c1)+ERF(c2))
      if (c1.ge.0..and.c2.ge.0.) eft=0.5*(ERF(c2)-ERF(c1))
      eft=eft+0.5*(ERF(c4)-ERF(c3))
      c1s=c1*c1
      c2s=c2*c2
      c3s=c3*c3
      c4s=c4*c4
...
c---------------------------------------------------
      ...
      if (erg.ge.1) then
       ...
       write(12,3615)
       write(12,3620)
       write(12,3630)(en(n),gtsan(n),ssf(n),tsdn(n),ts(n),n=nngp1,1,-1)
	...
	elseif(isfnq.ge.2) then
      ...
      if (erg.ge.1) then
       ...
       write(12,3615)
       write(12,3620)
       write(12,3630)(en(n),gtsan(n),tssf(n),tsdn(n),ts(n),n=nngp1,1,-1)
       write(12,3635)gtmga,tmgs,totqdn,gtmg
       write(12,3611)
       write(12,3610)
       write(12,3615)
...
c------------------
      ...
      call sainf(nag,title,idd,id,erg,ast,eal)
      call sninf(nag,title,id,erg,ast,eal)
...
c=======================================================================
      ...
      subroutine sainf(nag,title,idd,id,erg,ast,eal)
...
c=======================================================================
      ...
      subroutine sninf(nal,title,id,erg,ast,eal)
...
c---------
	...
	character*7 title2
      ...
      character*1 amt
	integer erg
      dimension title(11),title2(11),jzm(20),azm(20), czm(9,20), c(14),
...
c------------------
      ...
      call input(title,idd,id,erg)
      call processor(title,idd,id,erg)
...
c=======================================================================
      ...
      subroutine input(title,idd,id,erg)
...
c=======================================================================
      ...
      subroutine processor(title,idd,id,erg)
...
c-------------------------------------------------------
  ...
  240  continue
       do 246 mm = 1,nal(k)
        ...
        if(eala(mm,k).gt.9.8)eala(mm,k)=9.8
        do 245 m = 1,nag
         fm=1.
...
c--------------------------------------------------------------
     ...
     1 apro,q,e,x,nzb,jzb,azb,czb,jps,jl,astab,nstabb,naga,erg)
...
c--------------------------------------------------------------
       ...
       title1(10)='duction'
       call neutron(title1,ntb,idb,atb,jp,ep,f,lzt,lat,lst,amt,el,
     1 apro,q,e,x,nzb,jzb,azb,czb,jps,jl,astbc,nstbcb,nagb,erg)
      endif
...
c--------------------------------------------------------------
     ...
     1elc,aproc,qc,ec,xc,nzc,jzc,azc,czc,jpsc,jlc,astbc,nstbcc,nagb,erg)
...
c=======================================================================
     ...
     1 apro,q,e,x,nz,jz,az,cz,jps,jl,ast,gtsan,nal,erg)
...
c=======================================================================
     ...
     a 'mg','al','si',' p',' s','cl','ar',' k','ca','sc','ti',' v','cr',
     b 'mn','fe','co','ni','cu','zn','ga','ge','as','se','br','kr','rb',
     c 'sr',' y','zr','nb','mo','tc','ru','rh','pd','ag','cd','in','sn',
     d 'sb','te',' i','xe','cs','ba','la','ce','pr','nd','pm','sm','eu',
     e 'gd','tb','dy','ho','er','tm','yb','lu','hf','ta',' w','re','os',
     f 'ir','pt','au','hg','tl','pb','bi','po','at','rn','fr','ra','ac',
     g 'th','pa',' u','np','pu','am','cm','bk','cf','es','fm','md','no',
     h 'lr','rf','db'/

      data amass /1.0079,4.0026,6.941,9.0121,10.811,12.0107,14.0067,
     a15.9994,18.9984,20.1797,22.9897,24.3050,26.9815,28.0855,30.9737,
     b32.066,35.4527,39.948,39.0983,40.078,44.9559,47.867,50.9415,
     c51.9961,54.9380,55.845,58.9332,58.6934,63.546,65.39,69.723,
     d72.61,74.9216,78.96,79.904,83.80,85.4678,87.62,88.9058,91.224,
     e92.9063,95.94,98.,101.07,102.9055,106.42,107.8682,112.411,114.818,
     f118.710,121.760,127.6,126.9044,131.29,132.9054,137.327,138.9055,
     g140.116,140.9076,144.24,145.,150.36,151.964,157.25,158.9253,162.5,
     h164.9303,167.26,168.9342,173.04,174.967,178.49,180.9479,183.84,
     i186.207,190.23,192.217,195.078,196.9665,200.59,204.3833,207.2,
     j208.9803,209.,217.,222.,223.,226.,227.,232.0381,231.0358,
     k238.0289,237.,244.,243.,247.,247.,251.,252.,257.,258.,259.,262.,
     l261.,262./
